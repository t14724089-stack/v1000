...
class AIManager:
    ...
    def generate_analysis(self, prompt: str, model: str = None, context: Dict[str, Any] = None) -> str:
        ...
        # O argumento 'context' pode ser usado para passar informações adicionais ao modelo.
        # Para compatibilidade com o método antigo, vamos mapeá-lo para kwargs se for um dicionário.
        
        kwargs_for_response = {}
        if context:
            kwargs_for_response.update(context)

        # Se um modelo específico foi solicitado e está disponível, podemos priorizá-lo
        # No entanto, a lógica de fallback já está em generate_response
        
        # Chama generate_response com os parâmetros necessários
        response_data = self.generate_response(
            prompt=prompt,
            max_tokens=kwargs_for_response.get('max_tokens', 4000),
            temperature=kwargs_for_response.get('temperature', 0.7),
            system_prompt=kwargs_for_response.get('system_prompt'),
            **{k: v for k, v in kwargs_for_response.items() if k not in ['temperature', 'system_prompt', 'max_tokens']}
        )
        ...
        if response_data and response_data.get('success'):
            ...
            return f"Erro na geração: {response_data.get('error', 'Falha desconhecida')}"
    ...
    def generate_content(self, prompt: str, max_tokens: int = 1000, **kwargs) -> Optional[str]:
        ...
        return self.generate_analysis(prompt, model=provider, context={'max_tokens': max_tokens, **kwargs})
