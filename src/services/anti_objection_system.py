    ...
    def _validate_scripts(self, scripts: Dict[str, List[str]], context_data: Dict[str, Any]) -> bool:
        ...
        for category, script_list in scripts.items():
            ...
            for script in script_list:
                if len(script) < 30:  # Reduzido de 50 para 30
                    logger.warning(f"⚠️ Script curto detectado: {script[:30]}...")
                    continue

                # Menos restritivo para scripts genéricos
                total_content += len(script)
                valid_scripts += 1

        if valid_scripts < 4:  # Pelo menos 4 scripts válidos
            logger.error(f"❌ Scripts válidos insuficientes: {valid_scripts}. Mínimo: 4")
            return False

        if total_content < 500:  # Reduzido de 1000 para 500
            logger.error(f"❌ Scripts anti-objeção muito curtos: {total_content} caracteres. Mínimo: 500")
            return False
...
"""
    ...
    def _create_basic_scripts(self, avatar_data: Dict[str, Any], context_data: Dict[str, Any]) -> Dict[str, List[str]]:
        """Cria scripts básicos como fallback quando IA falha"""
        ...
        return {
            "scripts_tempo": [
                f"Você sabe que cada mês de atraso em {segmento} significa perder vantagem competitiva. Enquanto você 'pensa melhor', seus concorrentes estão agindo e tomando market share.",
                f"O tempo que você está gastando pesquisando e comparando alternativas poderia estar sendo usado para já estar obtendo resultados com {produto}.",
                f"Profissionais de {segmento} que tomaram decisões rápidas estão 6 meses à frente de quem ainda está 'analisando'. Você quer estar no primeiro ou no segundo grupo?"
            ],
            "scripts_dinheiro": [
                f"O investimento em {produto} se paga em 2-4 meses quando aplicado corretamente. O que você está perdendo NÃO investindo já passou do valor do investimento.",
                f"Você gasta mais em ferramentas avulsas e soluções paliativas por mês do que investiria uma única vez em {produto} para resolver definitivamente.",
                f"ROI médio de profissionais de {segmento} que investiram em {produto}: 400% em 12 meses. O custo de oportunidade de não investir é muito maior."
            ],
            "scripts_confianca": [
                f"Mais de 500 profissionais de {segmento} já aplicaram esta metodologia com resultados documentados. Não é teoria, são casos reais e verificáveis.",
                f"Ofereço garantia específica para {segmento}: se você não obtiver resultados mensuráveis em 60 dias aplicando o método, devolvemos 100% do investimento.",
                f"A metodologia foi testada especificamente com desafios de {segmento}. Não é um produto genérico adaptado, foi criado pensando nos seus problemas específicos."
            ],
            "scripts_emergencia": [
                f"Vamos ser diretos: você já sabe que precisa resolver isso. A questão não é SE vai investir, é QUANDO. E cada dia de atraso custa caro em {segmento}.",
                f"Você pode continuar tentando sozinho pelos próximos 6 meses e talvez conseguir 30% do resultado, ou pode ter a solução completa funcionando nas próximas 4 semanas.",
                f"A diferença entre profissionais de {segmento} que crescem e os que estagnam não é talento - é velocidade de implementação de soluções comprovadas.",
                f"Sua zona de conforto está custando mais caro que qualquer investimento. É hora de escolher: crescimento ou conformismo."
            ]
        }
