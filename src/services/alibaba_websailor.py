...
class AlibabaWebSailorAgent:
    ...
    def _extract_with_multiple_strategies(self, url: str) -> Optional[str]:
        ...
        strategies = [
            ("Jina Reader", self._extract_with_jina_reader), # Alterado para usar o método renomeado
    ...
    def _extract_with_jina_reader(self, url: str) -> Optional[str]:
        """Extrai conteúdo usando Jina Reader API com fallback"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Saldo insuficiente - usar fallback
                logger.warning(f"⚠️ Jina Reader: saldo insuficiente ou limite atingido para {url} - usando fallback.")
                return self._extract_with_fallback_methods(url)
            else:
    ...
    def _extract_with_fallback_methods(self, url: str) -> Optional[str]:
        """Métodos de fallback quando Jina Reader falha"""

        # Tenta Trafilatura primeiro
        content = self._extract_with_trafilatura(url)
        if content and len(content) > 100: # Ajustado o limite mínimo de caracteres
            logger.info(f"✅ Fallback (Trafilatura): {len(content)} caracteres de {url}")
            return content

        # Tenta Readability
        content = self._extract_with_readability(url)
        if content and len(content) > 100: # Ajustado o limite mínimo de caracteres
            logger.info(f"✅ Fallback (Readability): {len(content)} caracteres de {url}")
            return content

        # Tenta BeautifulSoup básico
        content = self._extract_with_beautifulsoup(url)
        if content and len(content) > 100: # Ajustado o limite mínimo de caracteres
            logger.info(f"✅ Fallback (BeautifulSoup): {len(content)} caracteres de {url}")
            return content

        logger.warning(f"⚠️ Todos os métodos de extração falharam para {url}")
        return None
